...
$.widget('custom.SuperGrid', {
    ...
    _renderGrid: function() {
        ...
        this._addMetaData();
        this._scrollBind(); //rebind this every time the grid is rendered because scrolls are not delegateable.
        this._trigger('supergrid-rendered');
    },

    _scrollBind: function(){
      var context = this;
      this.element.find('.supergrid_body').scroll(function(e) {
          console.log('scrolltriggered');
          var $gridBody = context.element.find('.supergrid_body'),
              amountScrolled = $gridBody.scrollTop(),
              gridViewportHeight = $gridBody.height(),
              widgetHeight = context.element.find('.supergrid_container').height();
          console.group();
          console.log('Amount Scrolled: ' + amountScrolled);
          console.log('gridView height: ' + gridViewportHeight);
          console.log(parseInt(amountScrolled) + parseInt(gridViewportHeight));
          console.log('Widget Height:' + widgetHeight);
          console.groupEnd();
          if (gridViewportHeight + amountScrolled == widgetHeight) {
              context.element.find('button.right').click();
              $gridBody.scrollTop(1);
              // Display alert or whatever you want to do when you're
              //   at the bottom of the page.
          } if (amountScrolled == 0) {
            //console.log((widgetHeight - gridViewportHeight) - 1);
            context.element.find('button.left').click();
    ...
    _buildBody: function() {
        ...
        this.options._grid.push('  <div class="supergrid_container">')
